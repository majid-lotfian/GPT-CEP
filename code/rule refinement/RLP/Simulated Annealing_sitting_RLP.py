...
class ThresholdOptimizerSA(Annealer):
    ...
    def activity_recognition(self, X, thresholds):
        predictions = []
        for x in X:
            # Implement the custom pattern for sitting activity
            if x[2] < thresholds[0] and x[4] < thresholds[1] and x[19] < thresholds[2] and x[34] < thresholds[3]:
                predictions.append(2)  # Sitting activity
            else:
    ...
    def energy(self):
        ...
        true_positives = np.sum((self.y_test == 2) & (y_pred == 2))
        false_positives = np.sum((self.y_test != 2) & (y_pred == 2))
        false_negatives = np.sum((self.y_test == 2) & (y_pred != 2))
...
# Initial thresholds for sitting activity
initial_state = [90, 3, 3, 3]
...
initial_true_positives = np.sum((y_test == 2) & (initial_thresholds_pred == 2))
initial_false_positives = np.sum((y_test != 2) & (initial_thresholds_pred == 2))
initial_false_negatives = np.sum((y_test == 2) & (initial_thresholds_pred != 2))
...
best_true_positives = np.sum((y_test == 2) & (best_thresholds_pred == 2))
best_false_positives = np.sum((y_test != 2) & (best_thresholds_pred == 2))
best_false_negatives = np.sum((y_test == 2) & (best_thresholds_pred != 2))
...
print(f'Classification Report for Sitting Activity:\n{classification_report(y_test, best_thresholds_pred)}')
...
plt.ylabel('Accuracy')
plt.title('Improvement in Accuracy over Iterations (Sitting Activity)')

plt.subplot(1, 2, 2)
plt.plot(annealer.f1_history)
plt.xlabel('Iteration')
plt.ylabel('F1 Score')
plt.title('Improvement in F1 Score over Iterations (Sitting Activity)')
