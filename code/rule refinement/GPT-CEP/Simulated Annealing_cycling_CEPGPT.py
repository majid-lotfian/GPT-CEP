...
class ThresholdOptimizerSA(Annealer):
    ...
    def activity_recognition(self, X, thresholds):
        predictions = []
        for x in X:
            # Implement the custom pattern for cycling activity
...
# Initial thresholds for cycling activity
initial_state = [120, 8, 8, 8]
...
plt.ylabel('Accuracy')
plt.title('Improvement in Accuracy over Iterations (Cycling Activity)')
