...
class ThresholdOptimizerSA(Annealer):
    def __init__(self, state, X_train, y_train, X_test, y_test, selected_features):
    ...
    def activity_recognition(self, X, thresholds):
        predictions = []
        for x in X:
            # Check selected features and their corresponding thresholds
            prediction = 0
            for i, feature_idx in enumerate(self.selected_features):
                if x[feature_idx] < thresholds[i]:  # Apply threshold on selected features
                    prediction = 6  # Detected cycling activity
                else:
                    prediction = 0  # Other activities
                break  # You can modify this logic based on how thresholds should combine for detection
            predictions.append(prediction)
        return np.array(predictions)

    def move(self):
        # Randomly change a threshold value
...
# Randomly select features and assign initial thresholds
num_features = random.randint(1, len(X_train[0]))  # Random number of features
selected_features = random.sample(range(X_train.shape[1]), num_features)  # Random feature indices
initial_state = [random.uniform(0, 20) for _ in range(num_features)]  # Random initial thresholds

# Create an instance of the ThresholdOptimizerSA class
annealer = ThresholdOptimizerSA(initial_state, X_train, y_train, X_test, y_test, selected_features)
...
best_true_positives = np.sum((y_test == 6) & (best_thresholds_pred == 6))
best_false_positives = np.sum((y_test != 6) & (best_thresholds_pred == 6))
best_false_negatives = np.sum((y_test == 6) & (best_thresholds_pred != 6))
...
print(f'Classification Report for Cycling Activity:\n{classification_report(y_test, best_thresholds_pred)}')
...
plt.ylabel('F1 Score')
plt.title('Improvement in F1 Score over Iterations (Cycling Activity)')
