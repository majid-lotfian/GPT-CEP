...
class ThresholdOptimizerSA(Annealer):
    ...
    def activity_recognition(self, X, thresholds):
        predictions = []
        for x in X:
            # Implement the custom pattern for running activity
            if x[2] < thresholds[0] and x[4] < thresholds[1] and x[19] < thresholds[2] and x[34] < thresholds[3]:
                predictions.append(5)  # Running activity
            else:
    ...
    def energy(self):
        ...
        true_positives = np.sum((self.y_test == 5) & (y_pred == 5))
        false_positives = np.sum((self.y_test != 5) & (y_pred == 5))
        false_negatives = np.sum((self.y_test == 5) & (y_pred != 5))
...
# Initial thresholds for running activity
initial_state = [140, 10, 10, 10]
...
initial_true_positives = np.sum((y_test == 5) & (initial_thresholds_pred == 5))
initial_false_positives = np.sum((y_test != 5) & (initial_thresholds_pred == 5))
initial_false_negatives = np.sum((y_test == 5) & (initial_thresholds_pred != 5))
...
best_true_positives = np.sum((y_test == 5) & (best_thresholds_pred == 5))
best_false_positives = np.sum((y_test != 5) & (best_thresholds_pred == 5))
best_false_negatives = np.sum((y_test == 5) & (best_thresholds_pred != 5))
...
print(f'Classification Report for Running Activity:\n{classification_report(y_test, best_thresholds_pred)}')
...
plt.ylabel('Accuracy')
plt.title('Improvement in Accuracy over Iterations (Running Activity)')

plt.subplot(1, 2, 2)
plt.plot(annealer.f1_history)
plt.xlabel('Iteration')
plt.ylabel('F1 Score')
plt.title('Improvement in F1 Score over Iterations (Running Activity)')
