...
class ThresholdOptimizerSA(Annealer):
    ...
    def activity_recognition(self, X, thresholds):
        predictions = []
        for x in X:
            # Implement the custom pattern for walking activity
            if x[2] < thresholds[0] and x[4] < thresholds[1] and x[19] < thresholds[2] and x[34] < thresholds[3]:
                predictions.append(4)  # Walking activity
            else:
    ...
    def energy(self):
        ...
        true_positives = np.sum((self.y_test == 4) & (y_pred == 4))
        false_positives = np.sum((self.y_test != 4) & (y_pred == 4))
        false_negatives = np.sum((self.y_test == 4) & (y_pred != 4))
...
# Initial thresholds for walking activity
initial_state = [140, 5, 5, 5]
...
initial_true_positives = np.sum((y_test == 4) & (initial_thresholds_pred == 4))
initial_false_positives = np.sum((y_test != 4) & (initial_thresholds_pred == 4))
initial_false_negatives = np.sum((y_test == 4) & (initial_thresholds_pred != 4))
...
best_true_positives = np.sum((y_test == 4) & (best_thresholds_pred == 4))
best_false_positives = np.sum((y_test != 4) & (best_thresholds_pred == 4))
best_false_negatives = np.sum((y_test == 4) & (best_thresholds_pred != 4))
...
print(f'Classification Report for Walking Activity:\n{classification_report(y_test, best_thresholds_pred)}')
...
plt.ylabel('Accuracy')
plt.title('Improvement in Accuracy over Iterations (Walking Activity)')

plt.subplot(1, 2, 2)
plt.plot(annealer.f1_history)
plt.xlabel('Iteration')
plt.ylabel('F1 Score')
plt.title('Improvement in F1 Score over Iterations (Walking Activity)')
